...
describe('InMemoryAudioBufferRepository', () => {
  ...
  it('서로 다른 participant의 startedAtMs는 독립적으로 기록된다', async () => {
    const repo = new InMemoryAudioBufferRepository();
    await repo.append('abc12xyz', 's1', Buffer.from('A'));
    await repo.append('abc12xyz', 's2', Buffer.from('B'));
    await repo.markStarted('abc12xyz', 's1', 1000);
  ...
  it('consume 후 markStarted 기록도 폐기 — 다음 consume에 startedAtMs가 남지 않는다', async () => {
    const repo = new InMemoryAudioBufferRepository();
    await repo.append('abc12xyz', 's1', Buffer.from('A'));
    await repo.markStarted('abc12xyz', 's1', 1000);
    await repo.consume('abc12xyz');
  ...
  describe('drainAvailable', () => {
    const KEEP_LAST = 32_000; // 16kHz pcm_s16le의 1초 분량
    ...
    it('누적량이 keepLastBytes보다 크면 (누적-keepLast) bytes 만큼 drain, 끝은 남는다', async () => {
      ...
      expect(res.startMs).toBe(0);
      // 두번째 drain 호출 시 남은 KEEP_LAST도 그대로 (drain 0 → 새 데이터 없음)
    ...
    it('drain 후 consume은 잔여 KEEP_LAST 분만 audio로, startMs는 drain 끝 위치를 가리킨다', async () => {
      const repo = new InMemoryAudioBufferRepository();
      // 누적 KEEP_LAST + 5000 → drain 5000 (startMs=0) → 잔여 KEEP_LAST 위치는 5000 byte부터.
    ...
    it('drain 한 적 없으면 consume의 startMs는 0 또는 undefined', async () => {
      const repo = new InMemoryAudioBufferRepository();
    ...
    it('서로 다른 participant의 drain cursor는 독립적이다', async () => {
      const repo = new InMemoryAudioBufferRepository();
