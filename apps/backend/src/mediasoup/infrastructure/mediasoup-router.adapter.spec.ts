...
describe('MediasoupRouterAdapter', () => {
  it('createRoom 직후엔 router 1개만 묶이고 getRtpCapabilities가 동작한다 (lazy add)', async () => {
  ...
  it('assignParticipant는 capacity 안에서 같은 router에 할당된다', async () => {
  ...
  it('capacity 초과 시 새 router가 lazy하게 추가되고 다음 참가자는 그쪽에 할당된다', async () => {
    ...
    try {
      ...
      // router#1이 추가됐으니 getRouterFor도 동작.
  ...
  it('createRoom 미호출 회의에 assignParticipant하면 실패한다', async () => {
  ...
  it('releaseParticipant 후 빈 자리가 생기면 같은 router에 다음 참가자가 들어간다', async () => {
    const { adapter, cleanup } = await setup(2, 2);
    try {
      await adapter.createRoom('CODE1111');
  ...
  it('closeRoom 후 해당 회의의 모든 router가 closed 상태가 된다', async () => {
    const { adapter, cleanup } = await setup(2, 1);
    try {
      await adapter.createRoom('CODE1111');
      // capacity 초과로 router 2개 만들기
  ...
  it('getParticipantRouter는 assignParticipant 결과의 router와 동일 인스턴스를 돌려준다', async () => {
  ...
  it('assign 안 된 참가자의 getParticipantRouter는 throw 한다', async () => {
    const { adapter, cleanup } = await setup(1, 1);
    try {
      await adapter.createRoom('CODE1111');
  ...
  it('pipeProducerToAllRouters: 다른 router의 transport에서도 동일 producerId로 consume 가능해진다', async () => {
    // 실제 mediasoup native worker 두 개로 router 2개를 묶고,
    // router#0에 만든 producer를 router#1로 pipe → router#1의 transport.consume이 같은 id로 동작하는지 검증.
    const { adapter, cleanup } = await setup(2, 1);
    try {
      await adapter.createRoom('CODE1111');
      // capacity=1이라 두 명 입장하면 router 2개가 자동 생성된다.
      await adapter.assignParticipant('CODE1111', 's1');
      await adapter.assignParticipant('CODE1111', 's2');
      const router0 = adapter.getRouterFor('CODE1111', 0);
      const router1 = adapter.getRouterFor('CODE1111', 1);

      // router#0에 send transport + producer 생성
      ...
      // pipe 호출 전: router#1에서 producer 못 찾음
      ...
      // pipe 후: router#1의 transport.consume 동작
  ...
  it('worker 수보다 더 많은 router가 필요해져도 router 수는 worker 수로 cap 되고, 초과 참가자는 가장 한가한 router에 over-allocate 된다', async () => {
    // worker 1개 + capacity 1 → router 1개가 max. 두번째 참가자가 와도 같은 router에 over-allocate
    // (mediasoup가 worker 내 같은 producerId를 2개 router에 등록할 수 없어서 pipeToRouter가 channel handler 충돌로 깨짐).
    const { adapter, cleanup } = await setup(1, 1);
    try {
      await adapter.createRoom('CODE1111');
      const a = await adapter.assignParticipant('CODE1111', 's1');
  ...
  it('한 회의의 router 들은 모두 서로 다른 worker에 spawn 된다 (다중 방으로 round-robin idx가 어긋나도)', async () => {
    // 다중 방 시나리오에서 globally round-robin인 worker idx가 어긋나 같은 회의의 두 router가 같은 worker에 들어가면,
    // pipeToRouter가 같은 producerId를 한 worker의 두 router에 등록하려다 channel handler ID 충돌로 깨진다.
    // 그래서 회의별 worker affinity가 필요.
    ...
    try {
      ...
      // 회의 A의 router 4 개는 모두 다른 worker에.
  ...
  it('pipeProducerToAllRouters는 routersPerRoom === 1 이면 no-op (호출만 swallow)', async () => {
    const { adapter, cleanup } = await setup(1, 1);
    try {
      await adapter.createRoom('CODE1111');
