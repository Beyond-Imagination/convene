 ...
 *
 * `mongoose.createConnection(uri, { dbName })`으로 클러스터에 접속하며, 같은 URI를 공유하되 `dbName`만 분리한다.
 *
 * 본 모듈은 mongoose의 `Connection` 클래스를 직접 DI 토큰으로 쓴다.
 * `ReportRepository` 등 인프라 어댑터들은 같은 인스턴스를 공유하면서 자기 컬렉션에 대응하는 Model만 생성한다
 */
@Global()
@Module({
  providers: [
