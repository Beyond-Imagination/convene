...
describe('resolveCorsOrigins', () => {
  ...
  it('빈 문자열만 들어 있으면 디폴트 origin으로 fallback', () => {
    expect(resolveCorsOrigins({ CORS_ORIGIN: ',  , ' })).toEqual([DEFAULT_CORS_ORIGIN]);
  });

  it('중복 origin은 제거한다', () => {
    expect(resolveCorsOrigins({ CORS_ORIGIN: 'https://a.com, https://a.com' })).toEqual([
      'https://a.com',
    ]);
  });
});
