 ...
 * 줄이고 연결 지연을 단축한다.
 *
 * 비정상 종료(네트워크 끊김) 시 자동 재연결을 사용한다 — 일정 간격(jitter 포함)
 * 으로 backend 가 살아날 때까지 무한 재시도. 재연결 성공 후의 회의 재JOIN /
 * mediasoup transport 재구축은 각 ViewModel 의 `connect` 이벤트 핸들러에서 담당.
 */
