...
/*
 * 두 명의 참가자 시나리오 (실제 socket.io broadcast + mediasoup pipeline까지 검증).
 *
 * Chromium fake media device(`--use-fake-device-for-media-stream`)로 카메라/마이크 없이 video/audio track을 송출한다.
 * 두 개의 BrowserContext를 분리해 두 명의 참가자가 다른 세션(닉네임 + zustand store)으로 동시에 같은 회의에 입장하는 흐름을 재현.
 *
 * 검증 범위:
 *   - 양쪽 모두 자기 LocalVideoTile의 video.srcObject가 부착된다(getUserMedia 동작).
 *   - 두 번째 참가자(B)의 화면에서 A가 참가자 리스트에 즉시 노출된다
 *     (서버가 join 직후 PARTICIPANTS broadcast로 기존 참가자 목록을 전달).
 *   - 첫 번째 참가자(A)의 화면에서도 B가 PARTICIPANT_JOINED로 노출된다.
 *
 * 원격 RTP 실제 수신(`remote-video-tile`의 srcObject 부착)은 mediasoup ICE/DTLS가 호스트 OS 방화벽·UDP 라우팅에 의존해 CI 환경에서 결정적이지 않다.
 * v1 단계에선 frontend ↔ backend의 시그널링 / 참가자 목록 합치까지만 결정적으로 검증.
 */
...
const joinMeeting = async (page: Page, code: string, nickname: string) => {
  await page.goto('/');
  await page.locator('form[aria-label="join-form"]').getByLabel('회의 코드').fill(code);
  await page.locator('form[aria-label="join-form"]').getByLabel('닉네임').fill(nickname);
...
test.describe('두 참가자 - 시그널링 통합', () => {
  ...
  test('A 회의 만들기 → B 입장 → 양쪽 자기 비디오 + 상대방 참가자 노출', async () => {
    ...
    // getUserMedia로 디바이스를 잡고 LocalVideoTile에 video.srcObject가 부착된다.
    ...
    // 모든 video element가 muted (autoplay 정책 회피)
