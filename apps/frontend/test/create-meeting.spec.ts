 ...
 *
 * 이전엔 닉네임이 없어 `/meetings/{code}` 진입 직후 useMeetingViewModel이
 * `/`로 redirect 시켰지만, 이제는 store에 닉네임이 있어 페이지가 유지된다.
 *
 * NOTE: Next.js의 `#__next-route-announcer__`는 `role="alert"` 라 strict 모드
 * 에서 selector 충돌이 생긴다. 본 spec은 URL 변화만으로 판정한다.
 */
test.describe('회의 생성 (backend 통합)', () => {
  test('닉네임 입력 후 회의 만들기 클릭 → /meetings/{code} 로 이동한다', async ({ page }) => {
    await page.goto('/');
  ...
  test('닉네임을 비워둔 채 클릭하면 검증 에러로 URL이 유지된다', async ({ page }) => {
    await page.goto('/');
    await page.getByRole('button', { name: '회의 만들기' }).click();
    // 검증 alert가 form 내부에 한정되어 떠야 한다(Next route announcer와 분리).
    await expect(
